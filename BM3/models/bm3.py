...
class BM3(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if self.v_feat is not None:
            ...
            self.v_feature_extractor = FeatureExtractorModel(self.v_feat, self.v_feat.shape[1],
                                                             self.meta_feat, self.feat_embed_dim, t=self.t)

            # self.image_trs = nn.Linear(self.v_feat.shape[1], self.feat_embed_dim)
            # nn.init.xavier_normal_(self.image_trs.weight)
        if self.t_feat is not None:
            # self.text_embedding = nn.Embedding.from_pretrained(self.t_feat, freeze=True)
            self.t_feature_extractor = FeatureExtractorModel(self.t_feat, self.t_feat.shape[1],
                                                             self.meta_feat, self.feat_embed_dim, t=self.t)
            # self.text_trs = nn.Linear(self.t_feat.shape[1], self.feat_embed_dim)
            # nn.init.xavier_normal_(self.text_trs.weight)

        self.feature_fusion_model = FeatureFusionModel(self.meta_feat, self.feat_embed_dim)
    ...
    def calculate_loss(self, interactions):
        ...
        if self.t_feat is not None:
            t_feat_online = self.t_feature_extractor.student_model(self.t_feat)
            # t_feat_online = self.text_trs(self.text_embedding.weight)
        if self.v_feat is not None:
            v_feat_online = self.v_feature_extractor.student_model(self.v_feat)
            # print(v_feat_online.shape)
        ...
        t_class_loss, t_kd_loss, t_feature_loss = 0.0, 0.0, 0.0
        num_modal = 0
        if self.t_feat is not None:
            # 计算特征变换的loss
            t_student_feat, t_class_loss, t_kd_loss, t_feature_loss= self.t_feature_extractor(items.long())
            # t_feat_target = F.dropout(t_feat_target, self.dropout)
            ...
            num_modal += 1
        v_class_loss, v_kd_loss, v_feature_loss = 0.0, 0.0, 0.0
        if self.v_feat is not None:
            # 计算特征变换的loss
            v_student_feat, v_class_loss, v_kd_loss, v_feature_loss = self.v_feature_extractor(items.long())
            # v_feat_target = F.dropout(v_feat_target, self.dropout)
        ...
        kd_loss = (v_kd_loss + t_kd_loss)/ num_modal
        feature_loss = (v_feature_loss + t_feature_loss)/ num_modal

        fused_feature = torch.cat([t_student_feat, v_student_feat], dim=1)
        fusion_loss = self.feature_fusion_model(items.long(), fused_feature)

        # print(teacher_loss, feature_loss, fusion_loss)

        mfd_loss = self.ce_weight * (class_loss + fusion_loss) + self.kd_weight * (kd_loss + feature_loss)

        total_loss = (loss_ui + loss_iu).mean() + self.reg_weight * self.reg_loss(u_online_ori, i_online_ori) + \
               self.cl_weight * (loss_t + loss_v + loss_tv + loss_vt).mean() + mfd_loss
