...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            m_feat_file_path = os.path.join(dataset_path, config['meta_feature_file'])
            ...
            if os.path.isfile(t_feat_file_path):
                self.t_feat = torch.from_numpy(np.load(t_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
            self.meta_feat = torch.from_numpy(np.load(m_feat_file_path, allow_pickle=True)).type(torch.LongTensor).to(
                self.device)
