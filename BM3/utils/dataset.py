...
class RecDataset(object):
    ...
    def load_inter_graph(self, file_name):
        self.train_df = pd.read_csv(os.path.join(self.dataset_path, 'train.csv'), index_col=None, usecols=None)
        self.test_df = pd.read_csv(os.path.join(self.dataset_path, 'test.csv'), index_col=None, usecols=None)
    ...
    def split(self):
        ...
        #
        # # wrap as RecDataset
        dfs = [self.train_df, self.test_df, self.test_df]
