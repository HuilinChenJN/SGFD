...
class MAML(object):
    def __init__(self,
        ...
        self.ce_wight = ce_weight
        self.kd_weight = kd_weight
        self.t_decay = t_decay

        feature_dim_feat = int(self.imagefeatures.shape[1])
        textual_dim_feat = int(self.textualfeatures.shape[1])

        self.visual_extractor_model = \
            FeatureExtractorModel(self.imagefeatures, feature_dim_feat, self.t_decay, self.label_data, dim_latent=self.embed_dim)

        self.textual_extractor_model = \
            FeatureExtractorModel(self.textualfeatures, textual_dim_feat, self.t_decay, self.label_data, dim_latent=self.embed_dim)

        self.feature_fusion_model = FeatureFusionModel(self.label_data, self.embed_dim)
    ...
    def mlp_layer_1(self):
        visual_student_feature = self.visual_extractor_model.student_model(self.imagefeatures)
        textual_student_feature = self.textual_extractor_model.student_model(self.textualfeatures)
        return tf.layers.dense(
            inputs=tf.nn.l2_normalize(tf.concat([visual_student_feature, textual_student_feature], axis=1), dim=1),
    ...
    def feature_loss(self):
        ...
        if self.feature_projection is not None:
            ...
            print(self.item_embeddings.shape, self.feature_projection.shape)
    ...
    def embedding_loss(self):
        ...
        positive_items = tf.reshape(self.user_positive_items_pairs[:, 1], [-1, 1])
        item_nodes = tf.reshape(tf.concat([positive_items, self.negative_samples], axis=1), [-1])

        # compute the knowledge distillation
        visual_batch_feature, visual_class_loss, visual_kd_loss, visual_feature_loss = self.visual_extractor_model(item_nodes)
        textual_batch_feature, textual_class_loss, visual_kd_loss, textual_feature_loss = self.textual_extractor_model(item_nodes)
        feature_input = tf.concat([visual_batch_feature, textual_batch_feature], 1, name='input')
        fusion_loss = self.feature_fusion_model(item_nodes, feature_input)

        class_loss = (visual_class_loss + textual_class_loss) / 2
        kd_loss = (visual_kd_loss + visual_kd_loss) / 2
        feature_loss = (visual_feature_loss + textual_feature_loss) / 2

        dis_loss = self.ce_wight * (fusion_loss + class_loss) + self.kd_weight * (kd_loss + feature_loss)
...
def optimize(model, sampler, train, test, args):
    ...
    while True:
        ...
        for user_chunk in tqdm(toolz.partition_all(50, test_users), desc="Evaluation......"):
            # print('user_chunk', len(user_chunk))
        ...
        print("\nresult on test set: ndcg:{},recall:{},hr:{},pr:{}".format(ndcgs,recalls,hit_ratios,precisions))
        # start to early-stop validate
...
def parse_args():
    ...
    parser.add_argument('--ce_weight', type=float, default=0.1, help='Learning rate.')
    parser.add_argument('--kd_weight', type=float, default=0.1, help='Learning rate.')
    parser.add_argument('--t_decay', type=int, default=10, help='Learning rate.')
    parser.add_argument('--feature_l2_reg', type=float, default=1.0, help='feature_l2_reg')
    parser.add_argument('--gpu', nargs='?', default='2', help='gpu_id')
...
if __name__ == '__main__':
    ...
    textualfeatures, imagefeatures, labelData = dataset.textualfeatures, dataset.imagefeatures, dataset.meta_data
    # print(type(features))
